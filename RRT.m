...
for iter = 1:3000
    ...
    height = height+1;
    %Step 4: 将x_new插入树T 
    %提示：新节点x_new的父节点是x_nearg
    T.v(height).x = x_new(1);         % T是我们要做的树，v是节点，这里先把起始点加入到T里面来
    T.v(height).y = x_new(2); 
    T.v(height).xPrev = x_near(1);     % 起始节点的父节点仍然是其本身
    T.v(height).yPrev = x_near(2);
    T.v(height).dist=Delta;          %从父节点到该节点的距离，这里可取欧氏距离
...
if iter < 2000
    ...
    pathIndex = T.v(height).indPrev; % 终点加入路径
    j=0;
    while 1
